...
- [ ] **后端测试**
  - [ ] 引入 `pytest` + `pytest-django`，创建 `conftest.py`
  - [ ] API 集成测试（market / account / strategy / orders）
  - [ ] 策略引擎单元测试（因子计算、信号生成、回测）
  - [ ] 风控管理器测试（7 项风控规则）
  - [ ] OKX Client Mock 测试
  - [ ] Celery 任务测试
...
- [ ] **E2E 测试**
  - [ ] 引入 Playwright / Cypress
  - [ ] 关键用户流程测试（配置凭证 → 同步行情 → 创建策略 → 下单）
...
- [ ] **WebSocket 实时推送**
  - [ ] 后端：Django Channels / SSE 将 OKX WebSocket 数据推送到前端
  - [ ] 前端：Tickers 页面实时价格更新（替代当前 HTTP 轮询）
  - [ ] 前端：K 线页面实时更新最新 K 线
  - [ ] 前端：仪表盘实时数据卡片
  - [ ] 连接状态指示器实时反映 WS 状态
...
### Sprint 2（测试 & 运维，预计 1-2 周）
1. pytest 测试框架 + 核心 API 测试
2. Docker 容器化
3. 日志系统配置
